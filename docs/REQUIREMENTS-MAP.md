...
| `helm-check` | Helm lint + restricted-PSS | helm + PSS assert | chart | always blocking | aligned |
| `cluster-smoke` | kind deploy + health probe | kind + kubectl | chart+image | advisory until `SECURITY_SCAN_BLOCKING=true` | intentional ramp |
